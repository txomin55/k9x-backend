...
VALUES  -- Classe 1
       ('avsc-dog-1', '', 'WHITE_SWISS_SHEPHERD_DOG', 'Bali From House Black&White', NULL, NULL,
       ...
       ('avsc-dog-2', '', 'BORDER_COLLIE', 'Angel Do Sonho Do Cão', NULL, NULL,
        ...
        -- Classe 2
       ('avsc-dog-3', '', 'RIESENSCHNAUZER', 'Simply Black Despacito', NULL, NULL,
        'k9x.support@gmail.com', 'PT', 'Gugadogs Centro De Treino & Educação Canina', 'António Dias',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'MALE'),
       ('avsc-dog-4', '', 'UNKNOWN', 'Joy', NULL, NULL,
        'k9x.support@gmail.com', 'PT', 'Afeto', 'Manuel Cunha',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'FEMALE'),
       ('avsc-dog-5', '', 'GERMAN_SHEPHERD', 'Zidane Vom Team Alfa Dog', NULL, NULL,
        'k9x.support@gmail.com', 'PT', 'Afeto', 'Ana Mota Pereira',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'MALE'),
        -- Classe 3
       ('avsc-dog-6', '', 'BORDER_COLLIE', 'Mind The Dog Thor', NULL, NULL,
        'k9x.support@gmail.com', 'PT', 'A.V.Super Cães', 'Florinda Sampaio',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'MALE'),
       ('avsc-dog-7', '', 'BORDER_COLLIE', 'Hooked On Obedience De Xonnydeby', NULL, NULL,
        'k9x.support@gmail.com', 'ES', 'Obelution', 'Sonia Grande',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'FEMALE'),
       ('avsc-dog-8', '', 'LABRADOR_RETRIEVER', 'Dea Diva Vom Trogenbach', NULL, NULL,
        'k9x.support@gmail.com', 'ES', 'Justgolden', 'Manuela Prieto Gómez',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'FEMALE'),
       ('avsc-dog-9', '', 'WHITE_SWISS_SHEPHERD_DOG', 'Chita', NULL, NULL,
        'k9x.support@gmail.com', 'PT', 'Gugadogs Centro De Treino E Educação Canina', 'António Dias',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'FEMALE'),
        -- COBS
       ('avsc-dog-10', '', 'DOBERMANN', 'Vince Vermute D''Ikòskylo', NULL, NULL,
        'k9x.support@gmail.com', 'PT', 'A.V.Super Cães', 'Arnauld Jambart',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'MALE'),
       ('avsc-dog-11', '', 'MALINOIS', 'BM2- Blaze Du Shroom', NULL, NULL,
       ...
       ('avsc-dog-12', '', 'POODLE', 'Spirit L´Huere Blueu', NULL, NULL,
        ...
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'FEMALE'),
       ('avsc-dog-13', '', 'POODLE', 'Valentine´s Night Magic', NULL, NULL,
        'k9x.support@gmail.com', 'PT', 'A.V. Super Cães', 'Sally Tinkle',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'FEMALE'),
       ('avsc-dog-14', '', 'POODLE', 'Dream Shannara Heart Of Steel', NULL, NULL,
        'k9x.support@gmail.com', 'PT', 'Individual', 'Joana Santos',
        FLOOR(EXTRACT(EPOCH FROM now()) * 1000), FLOOR(EXTRACT(EPOCH FROM now()) * 1000), NULL, NULL, 'MALE'),
       ('avsc-dog-15', '', 'CANE_CORSO', 'Flor De Fidelium Cohors', NULL, NULL,
