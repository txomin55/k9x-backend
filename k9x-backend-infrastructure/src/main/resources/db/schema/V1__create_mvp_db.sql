...
--
-- k9x.competitions.source above already tells API rows apart from the ones an external ETL loaded.
-- This is the detail behind those EXTRACTION rows: one per edition, written by the ETL from a
-- hand-written event_metadata.json that sits next to the raw material. Without it the provenance
-- lives only in whoever downloaded the sheets, and is lost as soon as they forget.
