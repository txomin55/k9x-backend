...
class CreateDogServiceCaseTest {
    ...
    void throws_exception_when_not_organizer_and_owner_is_null() {
        assertThatThrownBy(() -> serviceCase.createDog("dog-1", "Rex", "img", "Lab", "id", null, "user-1", "team", "ES", false))
    ...
    void throws_exception_when_not_organizer_and_owner_does_not_match_user() {
        assertThatThrownBy(() -> serviceCase.createDog("dog-1", "Rex", "img", "Lab", "id", "other-user", "user-1", "team", "ES", false))
                .isInstanceOf(UnauthorizedResourceException.class);
    ...
    void creates_dog_when_not_organizer_and_owner_matches_user() {
        serviceCase.createDog("dog-1", "Rex", "img", "Lab", "id", "user-1", "user-1", "team", "ES", false);

        verify(createDogPersistencePort).createDog(eq("dog-1"), eq("Rex"), eq("img"), eq("Lab"), eq("id"), eq("user-1"), eq("user-1"), eq("team"), eq("ES"), anyLong());
    }

    @Test
    void creates_dog_when_organizer_regardless_of_owner() {
        serviceCase.createDog("dog-1", "Rex", "img", "Lab", "id", "other-user", "user-1", "team", "ES", true);

        verify(createDogPersistencePort).createDog(eq("dog-1"), eq("Rex"), eq("img"), eq("Lab"), eq("id"), eq("other-user"), eq("user-1"), eq("team"), eq("ES"), anyLong());
    }
}
