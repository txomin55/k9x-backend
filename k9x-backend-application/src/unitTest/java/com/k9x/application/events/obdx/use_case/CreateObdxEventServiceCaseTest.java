...
class CreateObdxEventServiceCaseTest {
    ...
    private CreateObdxEventServiceCase serviceCase;

    @BeforeEach
    void setUp() {
        serviceCase = new CreateObdxEventServiceCase(getStagePersistencePort, createObdxEventPersistencePort);
    }

    @Test
    void throws_exception_when_user_is_not_organizer() {
        assertThatThrownBy(() -> serviceCase.createEvent("event-1", "Event 1", "stage-1", "user-1", false))
    ...
    void creates_event_when_all_conditions_are_met() {
        ...
        verify(createObdxEventPersistencePort).createEvent(eq("event-1"), eq("Event 1"), eq("stage-1"), eq("user-1"), anyLong());
    }
}
