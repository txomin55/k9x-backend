...
import com.k9x.domain.aggregates.events.obdx.ClassificationCompetitorStatus;
...
public class GetObdxEventClassificationServiceCase {
    ...
    public GetObdxEventClassificationServiceCase(
    ...
    public FetchClassificationDTO getClassification(String eventId) {
        ...
        FetchClassificationDTO cached = classificationCacheManagerPort.getIfPresentAndValid(eventId, config.cacheEvictStrategy().getTtlSeconds());
        if (cached != null) {
            return cached;
        }
        ...
        FetchClassificationDTO dto = aggregateProjection(eventId, event, stage, config, rawRows);
        classificationCacheManagerPort.put(eventId, dto);
        return dto;
    }

    private FetchClassificationDTO aggregateProjection(String eventId, ObdxEvent event, Stage stage,
