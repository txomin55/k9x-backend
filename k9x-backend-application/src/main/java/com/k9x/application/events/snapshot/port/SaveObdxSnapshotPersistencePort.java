 ...
 * {@code obdx.event_competitors}, together with the snapshot marker row holding the OBDX classification
 * payload (only the heavy computed part; event metadata is recovered on read). All writes for an event happen
 * in a single transaction, so a failure leaves the event without a snapshot and it is retried on the next run.
 */
