...
import com.k9x.infrastructure.in.rest.endpoints.secured.users.SetNotificationsEnabled;
...
public class SecuredUserEndpointConfiguration {
    ...
    @Bean
    public SetNotificationsEnabled setNotificationsEnabled(
            SetNotificationsEnabledServiceCase setNotificationsEnabledServiceCase, UserInfoDTO userInfoDTO) {
        return new SetNotificationsEnabled(setNotificationsEnabledServiceCase, userInfoDTO);
    }

    @Bean
    public GetUserData getUserData(GetUserSubscriptionsServiceCase getUserSubscriptionsServiceCase,
                                   GetNotificationsEnabledServiceCase getNotificationsEnabledServiceCase,
                                   UserInfoDTO userInfoDTO) {
        return new GetUserData(getUserSubscriptionsServiceCase, getNotificationsEnabledServiceCase, userInfoDTO);
    }
}
